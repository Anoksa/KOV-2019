				...
				if (flag_if)
				{
					...
					else
					{
						...
						*(log.stream) << "\tcmp eax, " << table.idtable.table[table.lextable.table[i + 3].idxTI].id << "\n";
						...
						else if (table.lextable.table[i + 2].operation == LT::LOREQU)
						{
							*(log.stream) << "\tjle m" << num_of_points << "\n";
							*(log.stream) << "\tjge m" << num_of_points + 1 << "\n";
						}
						else if (table.lextable.table[i + 2].operation == LT::MOREQU)
						{
							*(log.stream) << "\tjge m" << num_of_points << "\n";
							*(log.stream) << "\tjle m" << num_of_points + 1 << "\n";
						}
					}
