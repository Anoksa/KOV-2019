...
namespace LexAnalysis
{
	...
	Table LexAnalysis(In::IN in, Log::LOG log)
	{
		...
		for (int i = 0; word[i] != NULL; i++, iLex++)
		{
			...
			FST::FST fstStrCmp(word[i], GRAPH_STRCMP);
			if (FST::execute(fstStrCmp))
			{
				LT::Entry entryLT = AddLex(entryLT, LEX_STRCMP, iID++, line, LEX_STYPE_NULL);
				LT::Add(lextable, entryLT);
				entryIT = {};
				_mbscpy(entryIT.id, word[i]);
				entryIT.idxfirstLE = iLex;
				entryIT.idtype = IT::S;
				_mbscpy(entryIT.visibility, emptystr);
				entryIT.iddatatype = STRCMP_TYPE;
				entryIT.value.params.count = STRCMP_PARAMS_COUNT;
				entryIT.value.params.types = new IT::IDDATATYPE[STRCMP_PARAMS_COUNT];
				for (int k = 0; k < STRCMP_PARAMS_COUNT; k++)
					entryIT.value.params.types[k] = IT::STRCMP_PARAMS[k];
				IT::Add(idtable, entryIT);
				entryIT = {};
				libFunc = true;
				continue;
			...
			if (FST::execute(fstLogOperator))
			{
				...
				case '<':
					...
					break;
				case '`':
					entryLT = AddLex(entryLT, LEX_LOGOPERATOR, LT_TI_NULLIDX, line, LEX_STYPE_NULL, -1, LT::LOREQU);
					break;
				case '~':
					entryLT = AddLex(entryLT, LEX_LOGOPERATOR, LT_TI_NULLIDX, line, LEX_STYPE_NULL, -1, LT::MOREQU);
					break;
				case '?':
